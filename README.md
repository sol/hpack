...
| `name` | · | | | | |
| `version` | · | `0.0.0` | | | |
| `language` | `default-language` | `Haskell2010` | In contrast to `Cabal`'s `default-language` this is a top-level field, not a per section field.  You can override it on a per section basis with e.g. `default-extensions: GHC2021`. | `language: GHC2021` | `0.34.8` |
| `synopsis` | · | | | | |
| `description` | · | | | | |
| `category` | · | | | | |
| `stability` | · | | | | |
