    ...
    context "with an internal-libraries section" $ do
      it "reads the internal libraries correctly" $ do
        withPackageConfig [i|
          ...
          (packageInternalLibraries >>> (`shouldBe` Map.fromList [("example-a", (section $ Library Nothing ["Foo"] [] []) {sectionSourceDirs = ["src1"]}), ("example-b", (section $ Library Nothing ["Bar"] [] ["base:Data.Monoid as Baz"]) {sectionDependencies = deps ["base"], sectionSourceDirs = ["src2"]})]))
      ...
      it "accepts explicit exposed-modules and other-modules" $ do
        withPackageConfig [i|
          ...
          (packageInternalLibraries >>> (`shouldBe` (Map.singleton "example-foo" (section $ Library Nothing ["Foo"] ["FooInternal"] []))))
      ...
      it "infers other-modules from explicit exposed-modules" $ do
        withPackageConfig [i|
          ...
          (packageInternalLibraries >>> (`shouldBe` (Map.singleton "example-foo" (section $ Library Nothing ["Foo"] ["FooInternal", "Paths_foo"] []) {sectionSourceDirs = ["src"]})))
      ...
      it "infers exposed-modules from explicit other-modules" $ do
        withPackageConfig [i|
          ...
          (packageInternalLibraries >>> (`shouldBe` (Map.singleton "example-foo" (section $ Library Nothing ["Foo"] ["FooInternal"] []) {sectionSourceDirs = ["src"]})))
      ...
      it "combines global build options with its specific build options" $ do
        withPackageConfig [i|
          ...
          |]
          (touch "src/Foo.hs")
          (packageInternalLibraries >>> (`shouldBe` (Map.singleton "example-foo" (section $ Library Nothing ["Foo"] [] []) {sectionGhcOptions = ["-O2", "-Wall"], sectionSourceDirs = ["src"]})))
